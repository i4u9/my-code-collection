...
bool     gen_result(String* elements, String* mod, uint32_t elements_quant, char* placeholder, uint32_t placeholder_quant);

uint32_t gen_random_num(uint32_t limit);
void     init_arr(int32_t* array, uint32_t size);
...
int main()
{
    ...
    if (!gen_result(elements, &model, elements_quant, &placeholder, placeholder_quant)) return EXIT_FAILURE;
...
char get_placeholder(FILE* modelfile)
{
    ...
    fseek(modelfile, 1, SEEK_CUR); // reads last char on the line, completely moving the file pointer to the 2nd line
...
bool gen_result(String* elements, String* mod, uint32_t elements_quant, char* placeholder, uint32_t placeholder_quant)
{
    ...
    init_arr(random_arr, elements_to_insert);
    ...
    {
        insert_string(mod, elements[random_arr[i]], strcspn(mod->buffer, placeholder));
    }

    FILE* result_f = fopen("result.txt", "w");
    if (!result_f)
    {
        fprintf(stderr, "error: failed to open result file\n");
        return false;
    }
    
    fprintf(result_f, "%s", mod->buffer);
